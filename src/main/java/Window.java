...
public class Window implements Runnable {
    ...
    private void initFirstGeneration() {
        int column = (int) (Math.random() * Config.HEIGHT);
        int row = (int) (Math.random() * (Config.WIDTH - (Config.WIDTH / 4)));
        int rowLength = row + ((int) (Math.random() * (Config.WIDTH / 4)));
        for (; row < rowLength; row++) {
            boxes[row][column].cell.status = Status.LIVE;
            boxes[row][column].setColor();
        }
    }
